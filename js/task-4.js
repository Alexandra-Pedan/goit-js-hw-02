...
function formatString (string, maxLength = 40) {
    const isstring = string.split('');

    if (isstring.length > maxLength) {
        isstring.length = maxLength;
        return isstring.join('') + ('...');
    } else if (isstring.length = maxLength || isstring.length < maxLength) {
        return isstring.join('');
